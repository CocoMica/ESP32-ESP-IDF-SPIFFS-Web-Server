...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        mqtt_subscribe_ota_functions(client);
        break;
    ...
    case MQTT_EVENT_DATA:
        ...
        {
            // theres one way this can happen. the remaining data packets of a topic has topic_len = 0.
            if (expect_undefined_len_OTA_data)
            {
                handle_mqtt_ota_commands(event);
            }
        }
        else
        {
            if (startsWith(event->topic, "/ota/"))
            {
                handle_mqtt_ota_commands(event);
            }
            else
            {
                ESP_LOGW(TAG, "Unhandled MQTT_EVENT_DATA");
